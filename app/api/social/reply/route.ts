...
async function publishSocialUpdate(payload: SocialUpdatePayload) {
  try {
    ...
    console.log('[social-reply] published social-update:', payload.platform, payload.conversationId);
  } catch (err) {
    console.error('[social-reply] Redis publish failed:', err instanceof Error ? err.message : err);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const normalizedAttachments = (attachments ?? []).filter((a) => Boolean(a.url));
    ...
    switch (platform) {
      case 'facebook':
        result = await sendFacebookReply({
          messageId, messageType, recipientId,
          text: normalizedText,
    ...
    const persistedMessage = await persistOutgoingReply({
      platform, messageId, messageType, conversationId,
      recipientId, text: normalizedText,
      attachments: normalizedAttachments, result,
    });
    ...
    await publishSocialUpdate({
      type: 'social-update',
      platform,
      conversationId: baseConversationId ?? null,
...
async function sendFacebookReply({
  messageId, messageType, recipientId, text, attachments,
}: {
  messageId?: string;
  messageType?: string;
  recipientId?: string;
  ...
  if (messageType === 'comment') {
    ...
    if (!messageId) throw new Error('Facebook comment reply requires messageId');

    const response = await fetch(`https://graph.facebook.com/v18.0/${messageId}/comments`, {
      method: 'POST',
  ...
  if (!recipientId) throw new Error('Facebook message reply requires recipientId');
  ...
  return { id: lastResult?.id, message_id: lastResult?.message_id, deliveries: results };
}

async function sendInstagramReply(messageId: string | undefined, text: string) {
  const accessToken = process.env.INSTAGRAM_ACCESS_TOKEN;
  if (!messageId) throw new Error('Instagram reply requires messageId');

  const response = await fetch(`https://graph.facebook.com/v18.0/${messageId}/replies`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ message: text, access_token: accessToken }),
  });
...
async function sendWhatsAppReply(conversationId: string | undefined, text: string) {
  ...
  if (!conversationId) throw new Error('WhatsApp reply requires conversationId');
...
async function sendYouTubeReply(commentId: string | undefined, text: string) {
  ...
  if (!commentId) throw new Error('YouTube reply requires commentId');
...
async function persistOutgoingReply({
  platform, messageId, messageType, conversationId, recipientId, text, attachments, result,
  ...
  const content =
    text ||
    `[${(attachments ?? []).map((a) => a.type).join(', ')} attachment${(attachments ?? []).length > 1 ? 's' : ''}]`;
  const normalizedAttachments: SocialAttachmentInput[] = (attachments ?? []).map((a, i) => ({
    externalId: `${externalId || 'outgoing'}-${i}`,
    type: a.type,
  ...
  if (platform === 'facebook') {
    ...
    return await persistSocialMessage({
      platform, type: messageType === 'comment' ? 'comment' : 'message',
      externalId, conversationId: baseConversationId,
      senderId: pageProfile.id, senderName: pageProfile.name, senderAvatar: pageProfile.avatar,
      content,
      rawPayload: { request: { messageId, messageType, conversationId, recipientId, text, attachments }, response: result },
      isIncoming: false, isRead: true, timestamp: new Date(),
      attachments: normalizedAttachments,
    });
  }
  ...
  return await persistSocialMessage({
    platform, type: messageType === 'comment' ? 'comment' : 'message',
    externalId, conversationId: baseConversationId,
    senderId: `${platform}:admin`, senderName: 'Minsah Beauty',
    content,
    rawPayload: { request: { messageId, messageType, conversationId, recipientId, text, attachments }, response: result },
    isIncoming: false, isRead: true, timestamp: new Date(),
    attachments: normalizedAttachments,
